...
4. **Software** (`registry/software/`) — Pre-built WASM command binaries (coreutils, grep, sed, etc.) compiled from Rust and C source in `registry/native/`. See `registry/AGENTS.md` for naming conventions, package types, and how to add new packages.
...
- **Agent prompt injection must be non-destructive.** Each agent has its own mechanism for loading instructions (CLI flags, env vars, config files). When injecting OS instructions: preserve the agent's existing user-provided instructions (AGENTS.md, AGENTS.md, etc.), append rather than replace, and always provide `skipOsInstructions` opt-out. User configuration is never clobbered — user env vars override ours via spread order.
...
- **Keep `docs/fork-changes.md` up to date.** Track fork-only PRs on `0xpolarzero/agent-os`, including status, links, and a short summary of what each addition changes on the fork.
