 ...
 */
export function createHostDirBackend(options) {
    ...
    function resolve(p) {
        ...
        try {
            ...
            if (real !== canonicalRoot &&
                !real.startsWith(`${canonicalRoot}/`)) {
                throw new KernelError("EACCES", `path escapes host directory: ${p}`);
            }
            return real;
        }
        catch (err) {
    ...
    const backend = {
        ...
        async symlink(_target, _linkPath) {
            throw new KernelError("ENOSYS", "symlink not supported by host-dir backend");
        },
        async readlink(p) {
        ...
        async link(_oldPath, _newPath) {
            throw new KernelError("ENOSYS", "link not supported by host-dir backend");
        },
        async chmod(p, mode) {
            throwIfReadOnly();
