import { camelToKebab, getFieldInfos, getZodEnumValues, } from "./host-tools-argv.js";
/**
...
export function generateToolReference(toolKits) {
    ...
    lines.push("");
    lines.push("Run `agentos list-tools` to see all available tools.");
    lines.push("");
    for (const tk of toolKits) {
        ...
        for (const [toolName, tool] of Object.entries(tk.tools)) {
            ...
            lines.push(`- \`agentos-${tk.name} ${toolName}${flagStr}\` — ${tool.description}`);
        }
        lines.push("");
        ...
        if (toolsWithExamples.length > 0) {
            ...
            for (const [toolName, tool] of toolsWithExamples) {
                for (const ex of tool.examples ?? []) {
                    ...
                    lines.push(`- ${ex.description}: \`agentos-${tk.name} ${toolName}${flagArgs ? ` ${flagArgs}` : ""}\``);
                }
            }
            lines.push("");
        }
        lines.push(`Run \`agentos-${tk.name} <tool> --help\` for details.`);
        lines.push("");
    }
    return lines.join("\n");
}
