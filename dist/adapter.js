...
class PiSdkAgent {
    ...
    async newSession(params) {
        ...
        const { session } = await createAgentSession({
            ...
            ...(appendSystemPrompt
                ? {
                    resourceLoader: new (await import("@mariozechner/pi-coding-agent")).DefaultResourceLoader({
                        cwd: params.cwd,
                        appendSystemPrompt,
                    }),
                }
    ...
    async setSessionMode(params) {
        ...
        const matchedMode = availableModes.find((modeId) => modeId === params.modeId);
        if (!matchedMode) {
    ...
    async setSessionConfigOption(params) {
        ...
        if (params.configId === THOUGHT_LEVEL_CONFIG_ID) {
            const thinkingLevel = this.requireSelectValue(params, THOUGHT_LEVEL_CONFIG_ID);
            const availableLevels = session.getAvailableThinkingLevels();
            const matchedLevel = availableLevels.find((level) => level === thinkingLevel);
            if (!matchedLevel) {
                throw RequestError.invalidParams({
        ...
        throw RequestError.invalidParams({ configId: params.configId }, `Unsupported config option "${params.configId}"`);
    }
    async authenticate(_params) {
        // Auth handled via env vars (ANTHROPIC_API_KEY)
    }
    async buildModelState(session) {
        if (!session.model)
            return undefined;
        const currentModel = session.model;
    ...
    async resolveModel(modelId, session) {
        ...
        if (slashIndex !== -1) {
            ...
            const match = resolvedSession.modelRegistry.find(provider, providerModelId);
            if (!match) {
                throw RequestError.invalidParams({ modelId }, `Unknown model: ${modelId}`);
            }
            const apiKey = await resolvedSession.modelRegistry.getApiKey(match);
            if (!apiKey) {
                throw RequestError.invalidParams({ modelId }, `Model is unavailable without credentials: ${modelId}`);
            }
            return match;
        }
        ...
        if (matches.length > 1) {
            throw RequestError.invalidParams({ modelId }, `Ambiguous model ID "${modelId}". Use "provider/modelId" instead.`);
        }
        return matches[0];
    }
    // ── Event translation ───────────────────────────────────────────
    requireSession(sessionId) {
        ...
        if (!session) {
            throw RequestError.invalidRequest({ sessionId }, "No active session");
        }
    ...
    requireSelectValue(params, configId) {
        if ("type" in params && params.type === "boolean") {
            throw RequestError.invalidParams({ configId, value: params.value }, `Config option "${configId}" expects a select value`);
        }
        return String(params.value);
    }
    async setModelConfigOption(session, requestedModelId) {
        const currentModel = session.model;
        if (currentModel && toAcpModelId(currentModel) === requestedModelId) {
            return {
    ...
    async emitModeAndConfigUpdates(session, configOptions) {
        const resolvedConfigOptions = configOptions ?? (await this.buildConfigOptions(session));
    ...
    buildModeState(session) {
        const availableModes = session.getAvailableThinkingLevels();
        const currentModeId = availableModes.find((modeId) => modeId === session.thinkingLevel) ??
            availableModes[0] ??
            "off";
        return {
            currentModeId,
    ...
    async buildConfigOptions(session, modelState) {
        const configOptions = [];
        const resolvedModelState = modelState ?? (await this.buildModelState(session));
        ...
        const availableLevels = session.getAvailableThinkingLevels();
        const currentThinkingLevel = availableLevels.find((level) => level === session.thinkingLevel) ??
            availableLevels[0] ??
            "off";
        configOptions.push({
            type: "select",
