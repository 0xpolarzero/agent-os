...
class PiSdkAgent {
    ...
    async unstable_setSessionModel(params) {
        ...
        if (!model) {
            throw RequestError.invalidParams({ modelId: requestedModelId }, `Unknown model "${requestedModelId}"`);
        }
    ...
    setModelConfigOption(session, requestedModelId) {
        const currentModel = session.model;
        if (currentModel &&
            this.toAcpModelId(currentModel) === requestedModelId) {
        ...
        const model = this.resolveModelByAcpId(session, requestedModelId);
        if (!model) {
            throw RequestError.invalidParams({ configId: MODEL_CONFIG_ID, value: requestedModelId }, `Unknown model "${requestedModelId}"`);
        }
    ...
    emitConfigOptionsUpdate(session) {
        return this.emit({
            sessionUpdate: "config_option_update",
            configOptions: this.buildConfigOptions(session),
        });
    ...
    buildModelState(session) {
        ...
        return {
            ...
            availableModels: availableModels.map((model) => this.toModelInfo(model)),
        };
    }
    getSelectableModels(session) {
        const currentModel = session.model;
        const availableModels = session.modelRegistry.getAvailable();
        if (!currentModel) {
            return availableModels;
        }
        return availableModels.some((model) => this.modelsMatch(model, currentModel))
            ? availableModels
            : [currentModel, ...availableModels];
    }
    resolveModelByAcpId(session, modelId) {
        const [provider, ...modelIdParts] = modelId.split("/");
        const providerModelId = modelIdParts.join("/");
        if (!provider || !providerModelId) {
            return undefined;
        }
        return this.getSelectableModels(session).find((model) => model.provider === provider && model.id === providerModelId);
    }
    toModelInfo(model) {
        return {
